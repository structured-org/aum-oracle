...
use cosmwasm_std::{
    attr, entry_point, to_json_binary, Addr, Binary, Deps, DepsMut, Env, Int256, MessageInfo, Response, SignedDecimal256, StdError, StdResult
};
...
pub fn query_get_aum(deps: Deps, env: Env) -> ContractResult<GetAumResponse> {
    ...
    let aum_in_wbtc = aum_in_btc.atomics() / Int256::from_i128(10i128.pow(aum_in_btc.decimal_places()-WBTC_DECIMALS));

    Ok(GetAumResponse { aum_in_btc: aum_in_wbtc })
}
