...
TX=$(neutrond tx wasm store ./artifacts/jupiter_aum_receiver.wasm --from $FROM --keyring-backend $KEYRING --node $NODE --chain-id $CHAIN_ID --broadcast-mode sync --gas-prices $GAS_PRICES --gas auto --gas-adjustment 1.5 -y)
sleep 2
...
TX1=$(neutrond tx wasm store ./artifacts/binance_aum_receiver.wasm --from $FROM --keyring-backend $KEYRING --node $NODE --chain-id $CHAIN_ID --broadcast-mode sync --gas-prices $GAS_PRICES --gas auto --gas-adjustment 1.5 -y)
sleep 2
