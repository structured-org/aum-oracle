...
use cosmwasm_std::testing::message_info as std_message_info;
use cosmwasm_std::{
    from_json,
...
fn test_ownership() {
    ...
    let owner_info = std_message_info(&first_owner, &[]);
    let second_owner_info = std_message_info(&second_owner, &[]);
    let other_info = std_message_info(&other, &[]);
